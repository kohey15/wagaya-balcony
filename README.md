...
├ game.html           … ゲーム本編
├ about.html          … コンセプト説明
├ plants.html         … 植物図鑑（商品リンクは栄養解説を聞いた植物のみ解放）
...
- [x] ゲーム全体を5つの基本シーン（オープニング／栽培／植物選択／収穫物の解説／エンディング）で構成するよう整理（Ver0.2）
...
## 収穫後の栄養解説シーンの仕組み（毎ターン発生）

- 収穫があった日は必ず、収穫した植物のうち1つについて栄養素を紹介する短い会話が挟まる（以前はランダム35%だったが、Ver0.2後半で「毎ターン必ず」に変更）。
...
- 解説を聞いた植物IDは `state.explainedPlantIds` に記録される（`plants.html` の購入リンク解放に使用。後述）。

## 植物図鑑の購入リンク解放の仕組み

- `plants.html` の「本物の鉢を見てみる」リンクは、その植物の栄養解説シーンを一度でも聞いていないと表示されず、代わりに「ゲーム内で収穫して栄養解説を聞くと、購入リンクが解放されます。」という案内が出る。
- 判定は `state.explainedPlantIds`（localStorageのセーブデータ）を見て行う。つまり「買いたい苗があれば、その作物の栄養解説を実際にゲーム内で聞く」ことになり、栽培の心得や栄養についての学びと購入導線を結びつけている。
...
4. **収穫物の解説シーン**：収穫があった日は必ず挟まる、母と娘のキッチンでの栄養解説（毎ターン）。
5. **エンディングシーン**：全7種類の植物をコンプリート（＝ゲームクリア）した瞬間に1回だけ流れる、家族でのお祝いごはんの場面。ゲームを終了させるものではなく、その後も通常どおり遊び続けられる。
...
- エンディングシーンは `choosePlant()` で最後の1種を迎えた直後（`getUnownedPlantIds().length === 0` になった瞬間、＝ゲームクリア）に判定される（`state.endingShown` で一度きりに制御）。会話文言は `data/events.json` の `endingScene` から配列単位でランダムに選ばれる。
